...
RENITY_API Dimension2Dd Sprite::getImageScale() const {
    return pimpl_->scale;
}
...
RENITY_API Rect2Di Sprite::getImageClip() const {
    return pimpl_->srcRect;
}
...
RENITY_API double Sprite::getImageRotation() const {
    return pimpl_->rotation;
}
...
RENITY_API double Sprite::getMoveHeading() const {
    // atan2 normally takes (y, x), but we reverse them for the sake of going clockwise
    double angle = atan2(pimpl_->direction.x(), pimpl_->direction.y()) * radiansToDegrees;
    if (angle < 0.0)
        angle += 360.0;

    // Clear any domain errors that may have happened (i.e. passing (0, 0) to atan2).
    // The expectation is that it will simply have returned 0 in that case.
...
RENITY_API double Sprite::getMoveSpeed() const {
    return pimpl_->speed;
}
...
RENITY_API bool Sprite::draw() {
    if (pimpl_->tex) {
        return pimpl_->tex->draw(&pimpl_->srcRect, &pimpl_->destRect, pimpl_-> \
        rotation, &pimpl_->origin, pimpl_->flipHorizontal, pimpl_->flipVertical);
    }
