 ...
 * If you later need to disable the color key, just use SDL_SetColorKey directly.
 * @param surf The SDL_Surface on which to set the color key (transparent pixel value).
 * @param pos The (zero-indexed) position of the color key pixel.
 * @return 0 on success or a negative error code on failure (i.e. the return value of SDL_SetColorKey).
 */
 ...
 * @param destColor A destination pointer to set to the native-format color value. Automatically endianness-converted.
 ...
 * @param pos The (zero-indexed) position of the target pixel.
 * @param color The native-format color value to set. Automatically endianness-converted.
 * @return SDL_TRUE on success or SDL_FALSE on failure.
 */
 ...
 * @param a Pointer to be filled in with the Alpha component. Will be set to 0xff (100% opaque) if the pixel format doesn't support an alpha channel.
 * @return SDL_TRUE on success or SDL_FALSE on failure.
 */
 ...
 * @param a The Alpha component. Silently ignored if the pixel format doesn't support an alpha channel.
 * @return SDL_TRUE on success or SDL_FALSE on failure.
 */
