...
class RENITY_API Texture {
  public:
    ...
    /** Loader constructor.
     * Attempts to load a texture image using the given path. Check isValid() after.
     * \param window The Window whose renderer should be used for this Texture.
     ...
     * \returns True if a transparency color key pixel is enabled, false otherwise.
     */
     ...
     * \param origin The point around which to rotate dest, or NULL for the center.
    ...
    bool draw(const Rect2Di* source, const Rect2Di* dest, const double& angle = 0
            , const Point2Di* origin = nullptr, const bool& flipHorizontal = false
