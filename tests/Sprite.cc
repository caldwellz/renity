...
int main(int argc, char* argv[])
{
    ...
    assert(PHYSFS_mountMemory(epicArchive, epicArchiveLen, NULL, epicArchiveName, NULL, 1));
    ...
    printf("(%i, %i) -> (%i, %i), (%i, %i)\n", epicWidth / 2, epicHeight / 2, origin.x(), origin.y(), clip.width(), clip.height());
    fflush(NULL);
    ...
    assert(0.0 == sprite.getImageRotation());
    sprite.setImageRotation(45.5);
    assert(45.5 == sprite.getImageRotation());
    ...
    assert(0.0 == sprite.getMoveHeading());
    sprite.setMoveHeading(180.0);
    assert(180.0 == sprite.getMoveHeading());

    // Check direction get/set
    renity::Point2Dd direction = sprite.getMoveDirection();
    assert(0.0 == direction.x());
    assert(-1.0 == direction.y());
    ...
    for (Uint8 i = 0; i < 30; ++i) {
        sprite.move();
        assert(sprite.draw());
        SDL_SetRenderDrawColor(window.getRenderer(), 0, 0, 255, SDL_ALPHA_OPAQUE); // Pure blue
        window.update();
        SDL_Delay(50);
    }
